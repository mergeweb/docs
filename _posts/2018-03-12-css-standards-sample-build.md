...
Your basic file structure is already established with a some of the files in place already.
...
If you look at the markup, you'll see `class="container"` in a few different places. The container class is used to limit the maximum element to which it is applied.
The maximum width of container elements differs based on the width of the browser window.
Bootstrap's map variable that is used to control the max with of container elements at different screen sizes is called `$container-max-widths`.
Inside this map, different breakpoints are given maximum widths using a `key: value` structure.
...
Layout sections are the sections of the page that give it large-scale structure. Layout sections are typically globally visible across a site.
...
```
You'll see some BEM syntax used in the classes `page-header` (the block) and `page-header__inner` (an element in the page-header block).

To add some color to the header, we'll style the `page-header` class. Let's create a new file called `_page_header.sass` inside `css\src\layout\`.
...
Inside the `_page_header.sass` file, add the following styles to style the header.
...
The `page-header__inner` class is what we will use to get some positioning for the logo and the nav, with the logo on the left and nav on the right.
...
Add a `page-footer` class to the `<footer>` element.

```html
<footer class="page-footer">...</footer>
```

Create a new `_page_footer.sass` file in `css\src\layout`. Inside that file, add some styles to style the footer. 
...
We prefer to define any media query mixins in the component file itself. 

### The Profile cards
Time for the test! The tutorial branch has omitted the markup and styles for the faculty profile cards component.
Try using what you have learned to write some markup and styles for this component. Remember, identify the block and then identify the elements.
Add classes to your markup, write your styles and tweak as needed.

...

How'd you do? Below is one method of building this layout. If your markup or class names are not the same, that's OK.
Do evaluate, however, how you did with your BEM naming and identifying of elements!
